...
// Use these to find existing entities BEFORE creating new ones.
// This prevents duplicates and allows publish scripts to reuse entity IDs.

/**
 * Find an entity by exact name and type across all spaces.
...
export async function findEntityByName(
  ...
  const data = await gql(`{
    entities(
      typeId: "${typeId}"
      first: 10
      filter: { name: { includes: ${JSON.stringify(name)} } }
    ) { id name }
  }`);
  // The API uses "includes" not exact match, so verify exact name
  const match = data.entities?.find((e: any) => e.name === name);
  return match?.id ?? null;
}

/**
 * Find an entity by a unique text property value (e.g., Retrosheet ID).
 * Searches across all spaces.
 * Returns the entity ID if found, null otherwise.
 */
export async function findEntityByTextValue(
  propertyId: string,
  value: string,
): Promise<string | null> {
  const data = await gql(`{
    values(filter: {
      ...
      text: { is: ${JSON.stringify(value)} }
    }) { entityId }
  }`);
  return data.values?.[0]?.entityId ?? null;
}

/**
 * Find an entity by a unique integer property value (e.g., MLB ID).
 * Searches across all spaces.
 * Returns the entity ID if found, null otherwise.
 */
export async function findEntityByIntegerValue(
  propertyId: string,
  value: number,
): Promise<string | null> {
  const data = await gql(`{
...
/**
 * Find or confirm an entity exists by ID.
 * Returns the entity if found, null otherwise.
 */
export async function findEntityById(
  entityId: string,
): Promise<{ id: string; name: string; typeIds: string[] } | null> {
  const data = await gql(`{
    entity(id: "${entityId}") { id name typeIds }
  }`);
